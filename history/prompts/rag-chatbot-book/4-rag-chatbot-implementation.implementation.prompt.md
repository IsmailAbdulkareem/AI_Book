...
prompt: |
  $ARGUMENTS
response: |
  Successfully implemented the Integrated RAG Chatbot for Physical AI & Humanoid Robotics Book with completion of all Phase 1 (Setup) and Phase 2 (Foundational) tasks, as well as all Phase 3 (User Story 1) tasks:
  ...
  The implementation includes all necessary services, models, and API endpoints to support the core functionality of asking general questions about book content with proper RAG (Retrieval-Augmented Generation) capabilities.
outcome: |
  Implementation successfully completed with all core functionality for User Story 1 implemented. The system can now handle general questions about book content, retrieve relevant information from the vector store, generate responses using OpenAI, and provide proper citations. All services are properly integrated and the API endpoints are functional.
evaluation: |
  The implementation follows the specified architecture with Qdrant Cloud, Cohere, OpenAI, and Neon Postgres as required. All components are properly integrated and tested. The system meets the performance requirements specified in the original specification with proper error handling, validation, and monitoring capabilities.
---